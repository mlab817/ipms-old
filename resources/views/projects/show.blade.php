                                ...
                                <div class="Box-body">
                                    ...
                                    <form action="{{route('projects.update', $project)}}" method="POST">
                                        ...
                                        <dl class="form-group d-inline-block my-0">
                                            ...
                                            >
                                                <input x-show="!isEditing" @click="isEditing = true; $nextTick(() => $refs.total_project_cost.focus());" class="form-control" type="text" readonly x-bind:value="addComma(total_project_cost)">
                                                <div x-cloak>
                                                    <input x-show="isEditing" type="number" x-ref="total_project_cost" name="total_project_cost" id="total_project_cost" x-model="total_project_cost" class="form-control">
                                                    <button x-show="isEditing" x-bind:disabled="!total_project_cost" class="btn" type="submit">Save</button>
                                                    <button x-show="isEditing" @click="isEditing = false; $nextTick(() => (total_project_cost = {{ $project->total_project_cost }}));" class="btn" type="button">Cancel</button>
                                                </div>
                                            </dd>
                                        </dl>
                                    </form>
                                            ...
                                            <div class="card card-primary card-outline">
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <form action="{{ route('projects.update', $project) }}" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <div class="col-sm-9" x-data="{ isEditing: false }">
                                                            <textarea rows="4" style="resize: none;"
                                                                      x-ref="expected_outputs"
                                                                      @click="isEditing = true; $nextTick(() => (addEasyMDE($refs.expected_outputs)));"
                                                                      @click.away="isEditing = false"
                                                                      class="form-control @error('expected_outputs') is-invalid @enderror"
                                                                      id="expected_outputs"
                                                                      name="expected_outputs"
                                                                      x-bind:readonly="!isEditing">{{ old('expected_outputs', $project->expected_output->expected_outputs) }}</textarea>
                                                            <button type="submit" class="btn mt-2" x-show="isEditing">Submit</button>
                                                            <button type="button" class="btn mt-2" x-show="isEditing" @click="isEditing = false">Cancel</button>
                                                        </div>

                                                        <script>
                                                            // let textarea = this.$refs.expected_outputs;

                                                            function addEasyMDE(textarea) {
                                                                return new EasyMDE({ element: textarea, maxHeight: '120px' })
                                                            }

                                                            function removeEasyMDE(textarea) {
                                                                addEasyMDE(textarea).toTextArea().cleanup();
                                                            }
                                                        </script>
                                                    </form>
                                                </div>
