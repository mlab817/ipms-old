        ...
        <div class="container-lg d-flex flex-justify-between py-6">
            <div class="flex-auto">
                ...
                <p class="f4 color-text-secondary col-md-8">
                    PAPs to be transferred or to be received
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="container-lg py-6">
        <div class="tabnav">
            <nav class="tabnav-tabs" aria-label="Foo bar">
                <a class="tabnav-tab" href="{{ route('pending_transfers.index', ['completed' => 0]) }}" @if($completed == 0) aria-current="page" @endif>
                    ...
                    <span>Pending</span>
                    <span class="Counter">{{ $pendingCount }}</span>
                </a>

                <a class="tabnav-tab" href="{{ route('pending_transfers.index', ['completed' => 1]) }}" @if($completed == 1) aria-current="page" @endif>
                    <!-- <%= octicon "git-commit" %> -->
            ...
            <div class="Box">
                @forelse ($pendingTransfers as $item)
                    @if ($item->project)
                        <div role="row" class="Box-row d-flex">
                            <div class="d-flex flex-column flex-auto">
                                <div class="flex-auto mr-2">
                                    <div>
                                        ...
                                        <div>
                                            <span class="text-small color-text-tertiary">
                                                {{ first_sentence($item->project->description->description) }}
                                            </span>
                                        </div>
                                    </div>
                                    @if(! $item->completed)
                                        @if($item->from == auth()->id())
                                            <form action="{{ route('pending_transfers.destroy', $item) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @if($item->to == auth()->id())
                                            <details class="details-reset details-overlay ml-1 position-relative">
                                                <summary class="btn" aria-haspopup="true" role="button">
                                                    Action
                                                    <span class="dropdown-caret"></span>
                                                </summary>

                                                <div class="SelectMenu right-0">
                                                    <form action="{{ route('pending_transfers.update', $item) }}" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="SelectMenu-modal">
                                                            <div class="SelectMenu-list">
                                                                <button class="SelectMenu-item" role="menuitem" type="submit" name="action" value="accept">Accept</button>
                                                                <button class="SelectMenu-item" role="menuitem" type="submit" name="action" value="reject">Reject</button>
                                                            </div>
                                                        </div>
                                                    </form>
                                                </div>
                    ...
                    @else
                        <div role="row" class="Box-row d-flex flex-items-center">
                            <div class="flex-auto">
