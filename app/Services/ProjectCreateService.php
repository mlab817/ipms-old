...
class ProjectCreateService
{
    ...
    public function createRegionInvestments($regionInvestments)
    {
        ...
        $merged = $regions->map(function ($row) use ($regionInvestments) {
            $otherData = $regionInvestments->where('region_id', $row->id)->first();
            return collect($row)->concat($otherData);
        });
        dd($merged);
        ...
        foreach ($regions as $region) {
            if (!empty($regionInvestments)) {
                ...
                // use createMany
                $this->project->region_investments()->createMany([]);
            }
    ...
    public function createFsInvestments($investments)
    {
        ...
        foreach ($fundingSources as $fs) {
            if (!empty($investments)) {
                ...
                if ($fsToInsert) {
                    FsInvestment::withoutTouching(function () use ($fsToInsert) {
                        $this->project->fs_investments()->create($fsToInsert);
                    });
                } else {
                    FsInvestment::withoutTouching(function () use ($fs) {
    ...
    public function createFsInfrastructures($investments)
    {
        ...
        foreach ($fundingSources as $fs) {
            if (!empty($investments)) {
                ...
                if ($fsToInsert) {
                    FsInfrastructure::withoutTouching(function () use ($fsToInsert) {
                        $this->project->fs_infrastructures()->create($fsToInsert);
                    });
                } else {
                    FsInfrastructure::withoutTouching(function () use ($fs) {
